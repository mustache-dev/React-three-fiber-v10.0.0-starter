...
export const Slash = () => {
    ...
    const slashNodes = ({ progress }) => {
        ...
        const noise = texture(noiseTexture, vec2(uvX, uv().y).mul(float(0.4).add(progress.mul(0.3).mod(1))))
        ...
        const revealMask = smoothstep(revealCutoff.sub(revealEdgeSoftness), revealCutoff.add(revealEdgeSoftness), uvX.oneMinus())

        // Dissolve animation (second part of life - starts at 0.3 progress)
        const dissolveStart = float(0.3)
        const dissolveProgress = progress.sub(dissolveStart).div(float(1).sub(dissolveStart)).clamp(0, 1)
        ...
        const dissolveMask = smoothstep(dissolveThreshold.sub(0.05), dissolveThreshold.add(0.05), dissolveValue)
        ...
        const colorWithDissolveEdge = mix(baseColor, dissolveEdgeColor, dissolveEdgeIntensity.mul(dissolveMask))

        const backdropWithSlash = mix(distortedBackdrop, colorWithDissolveEdge, finalOpacity.sub(endFade).clamp(0, 1))
